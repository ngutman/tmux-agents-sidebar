...
controller kill-current >/dev/null
assert_eq "1" "$(session_option @agents_sidebar_wide_layout_dirty)" "killing a pane in compact mode should dirty the saved wide layout"
...
[[ "$layout_after" != "$layout_before" ]] || fail "wide restore should fall back after kill-current changes compact pane membership"

echo "ok - kill-current in compact mode marks the saved wide layout dirty"
