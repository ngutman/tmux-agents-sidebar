...
tmux set-hook -g after-kill-pane "run-shell -b 'AGENTS_SIDEBAR_SESSION=\"#{session_id}\" AGENTS_SIDEBAR_PANE=\"#{pane_id}\" $CONTROLLER_Q repair'"
tmux set-hook -g pane-exited "run-shell -b 'AGENTS_SIDEBAR_SESSION=\"#{session_id}\" AGENTS_SIDEBAR_PANE=\"#{pane_id}\" $CONTROLLER_Q repair'" 2>/dev/null || true
